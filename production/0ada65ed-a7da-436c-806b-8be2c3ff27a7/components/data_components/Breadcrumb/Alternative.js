const breadcrumbalternative = {
    ...
    mounted() {
        ...
        breadcrumbItems.forEach((breadcrumbItem, index) => {
            var tempPercent = (index + 1) / breadcrumbItemsLength;
            breadcrumbItem.style.background = 'color-mix(in srgb, var(--breadcrumb-bg-color) ' + (tempPercent * 100) + '%, white)';
            breadcrumbItem.style.color = 'color-mix(in srgb, var(--breadcrumb-text-color) ' + (tempPercent * 100) + '%, white)';
        });
    },
    created: function () {
...
app.component('breadcrumbalternative', {
    extends: breadcrumbalternative,
    template: '#breadcrumbalternative'
});